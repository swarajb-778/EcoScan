<script lang="ts">
  ...
  async function performDetectionWithOfflineSupport() {
    if (!videoElement || !canvasElement || !overlayCanvasElement) {
      console.error("❌ Camera elements not initialized");
      return;
    }
    
    if (!detector) {
      console.error("❌ Detector not initialized - attempting to reinitialize");
      try {
    ...
    try {
      ...
      if (videoElement.videoWidth === 0 || videoElement.videoHeight === 0) {
        console.warn("⚠️ Video dimensions are zero, skipping frame");
        return;
      }
      ...
      console.log(`📊 Canvas dimensions: ${canvasElement.width}x${canvasElement.height}, Video dimensions: ${videoElement.videoWidth}x${videoElement.videoHeight}`);
      ...
      if (frameCount % 100 === 0) {
        // Create performance summary without using unsupported methods
        const performanceReport = perf.snapshot();
  ...
  // Capture still image for analysis
  async function captureImage() {
    ...
    try {
      // Pause video stream and detection loop
      ...
      // Update detections store
      detections.set(capturedDetections);
      
      // Draw detection results
      drawDetections(capturedDetections);
      ...
      // Set success message
      setSuccess('Image captured and analyzed successfully');
...
<div class="relative w-full h-full min-h-[400px] bg-black rounded-2xl overflow-hidden">
  ...
  {#if browserPermissionsGranted && !browserIsLoading}
    ...
    <!-- Capture button -->
    <button 
      class="absolute bottom-24 left-1/2 transform -translate-x-1/2 bg-green-500 rounded-full p-4 shadow-lg hover:bg-green-600 active:bg-green-700 transition-all"
